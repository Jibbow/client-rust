// Copyright 2020 TiKV Project Authors. Licensed under Apache-2.0.

use crate::{Error, Result};
use async_trait::async_trait;
use grpcio::CallOption;
use std::any::Any;
use tikv_client_proto::{coprocessor_v2 as coprv2pb, kvrpcpb, tikvpb::TikvClient};

#[async_trait]
pub trait Request: Any + Sync + Send + 'static {
    async fn dispatch(&self, client: &TikvClient, options: CallOption) -> Result<Box<dyn Any>>;
    fn label(&self) -> &'static str;
    fn as_any(&self) -> &dyn Any;
    fn set_context(&mut self, context: kvrpcpb::Context);
}

macro_rules! impl_request {
    ($name: path, $fun: ident, $label: literal) => {
        #[async_trait]
        impl Request for $name {
            async fn dispatch(
                &self,
                client: &TikvClient,
                options: CallOption,
            ) -> Result<Box<dyn Any>> {
                client
                    .$fun(self, options)?
                    .await
                    .map(|r| Box::new(r) as Box<dyn Any>)
                    .map_err(Error::Grpc)
            }

            fn label(&self) -> &'static str {
                $label
            }

            fn as_any(&self) -> &dyn Any {
                self
            }

            fn set_context(&mut self, context: kvrpcpb::Context) {
                self.set_context(context);
            }
        }
    };
}

impl_request!(kvrpcpb::RawGetRequest, raw_get_async_opt, "raw_get");
impl_request!(
    kvrpcpb::RawBatchGetRequest,
    raw_batch_get_async_opt,
    "raw_batch_get"
);
impl_request!(kvrpcpb::RawPutRequest, raw_put_async_opt, "raw_put");
impl_request!(
    kvrpcpb::RawBatchPutRequest,
    raw_batch_put_async_opt,
    "raw_batch_put"
);
impl_request!(
    kvrpcpb::RawDeleteRequest,
    raw_delete_async_opt,
    "raw_delete"
);
impl_request!(
    kvrpcpb::RawBatchDeleteRequest,
    raw_batch_delete_async_opt,
    "raw_batch_delete"
);
impl_request!(kvrpcpb::RawScanRequest, raw_scan_async_opt, "raw_scan");
impl_request!(
    kvrpcpb::RawBatchScanRequest,
    raw_batch_scan_async_opt,
    "raw_batch_scan"
);
impl_request!(
    kvrpcpb::RawDeleteRangeRequest,
    raw_delete_range_async_opt,
    "raw_delete_range"
);
impl_request!(
    kvrpcpb::RawCasRequest,
    raw_compare_and_swap_async_opt,
    "raw_compare_and_swap"
);
impl_request!(
    coprv2pb::RawCoprocessorRequest,
    coprocessor_v2_async_opt,
    "raw_coprocessor_request"
);

impl_request!(kvrpcpb::GetRequest, kv_get_async_opt, "kv_get");
impl_request!(kvrpcpb::ScanRequest, kv_scan_async_opt, "kv_scan");
impl_request!(
    kvrpcpb::PrewriteRequest,
    kv_prewrite_async_opt,
    "kv_prewrite"
);
impl_request!(kvrpcpb::CommitRequest, kv_commit_async_opt, "kv_commit");
impl_request!(kvrpcpb::CleanupRequest, kv_cleanup_async_opt, "kv_cleanup");
impl_request!(
    kvrpcpb::BatchGetRequest,
    kv_batch_get_async_opt,
    "kv_batch_get"
);
impl_request!(
    kvrpcpb::BatchRollbackRequest,
    kv_batch_rollback_async_opt,
    "kv_batch_rollback"
);
impl_request!(
    kvrpcpb::PessimisticRollbackRequest,
    kv_pessimistic_rollback_async_opt,
    "kv_pessimistic_rollback"
);
impl_request!(
    kvrpcpb::ResolveLockRequest,
    kv_resolve_lock_async_opt,
    "kv_resolve_lock"
);
impl_request!(
    kvrpcpb::ScanLockRequest,
    kv_scan_lock_async_opt,
    "kv_scan_lock"
);
impl_request!(
    kvrpcpb::PessimisticLockRequest,
    kv_pessimistic_lock_async_opt,
    "kv_pessimistic_lock"
);
impl_request!(
    kvrpcpb::TxnHeartBeatRequest,
    kv_txn_heart_beat_async_opt,
    "kv_txn_heart_beat"
);
impl_request!(
    kvrpcpb::CheckTxnStatusRequest,
    kv_check_txn_status_async_opt,
    "kv_check_txn_status"
);
impl_request!(
    kvrpcpb::CheckSecondaryLocksRequest,
    kv_check_secondary_locks_async_opt,
    "kv_check_secondary_locks_request"
);
impl_request!(kvrpcpb::GcRequest, kv_gc_async_opt, "kv_gc");
impl_request!(
    kvrpcpb::DeleteRangeRequest,
    kv_delete_range_async_opt,
    "kv_delete_range"
);
